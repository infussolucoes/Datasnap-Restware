...
type
 ...
 function  ResultJsonValue(var JsonParser: TJsonParser): TJsonValue;
...
begin
  ...
  begin
    ...
    begin
      Inc(N);
      SetLength(Result, N);
      Result[N - 1] := ResultJsonValue(JsonParser);
      White(JsonParser);
      if JsonParser.Ch = ']' then
      begin
        Next(JsonParser, ']');
...
begin
  ...
  begin
    ...
    begin
      ...
      SetLength(Result, N);
      Result[N - 1].Key := Key;
      Result[N - 1].Value := ResultJsonValue(JsonParser);
      White(JsonParser);
      if JsonParser.Ch = '}' then
      begin
        Next(JsonParser, '}');
...
begin
  ...
  case JsonParser.Ch of
    ...
    begin
      ...
      Result.Index := N;
      Result.Value := String_(JsonParser);
    end;
    't', 'f', 'n':
    begin
      Result.Kind := JVKWord;
