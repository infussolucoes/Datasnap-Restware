...
{$IF CompilerVersion > 28}
    , System.NetEncoding, System.JSON, FireDAC.Stan.StorageJSON,
...
Type
  ...
  Private
    ...
    // Seta a Porta do Pooler a ser usada
    Procedure CheckConnection; // Checa o Estado automatico da Conexão
    Function TryConnect: Boolean;
    // Tenta Conectar o Servidor para saber se posso executar comandos
    Procedure SetConnectionOptions(Var Value: TDSRestConnection);
    // Seta as Opções de Conexão
    Function ExecuteCommand(Var SQL: TStringList; Var Params: TParams;
      Var Error: Boolean; Var MessageError: String; Execute: Boolean = False)
      : TFDJSONDataSets;
    Procedure ExecuteProcedure(ProcName: String; Params: TParams;
      Var Error: Boolean; Var MessageError: String);
    Procedure ApplyUpdates(Var SQL: TStringList; Var Params: TParams;
      ADeltaList: TFDJSONDeltas; TableName: String; Var Error: Boolean;
  ...
  Published
    Property OnConnection: TOnEventConnection Read vOnEventConnection
      Write vOnEventConnection;
    // Evento relativo a tudo que acontece quando tenta conectar ao Servidor
    Property OnBeforeConnect: TOnEventBeforeConnection Read vOnBeforeConnection
      Write vOnBeforeConnection; // Evento antes de Connectar o Database
    Property Active: Boolean Read vConnected Write SetConnection;
    // Seta o Estado da Conexão
    Property Compression: Boolean Read vCompression Write vCompression;
    // Compressão de Dados
    Property MyIP: String Read vMyIP;
    Property Login: String Read vLogin Write vLogin;
    // Login do Usuário caso haja autenticação
    Property Password: String Read vPassword Write vPassword;
    // Senha do Usuário caso haja autenticação
    ...
    Property PoolerURL: String Read vRestURL Write vRestURL;
    ...
    Property RestModule: String Read vRestModule Write vRestModule;
    // Classe do Servidor REST Principal
    Property StateConnection: TAutoCheckData Read vAutoCheckData
      Write vAutoCheckData; // Autocheck da Conexão
    Property RequestTimeOut: Integer Read vTimeOut Write vTimeOut;
    // Timeout da Requisição
    Property Encoding: TEncodeSelect Read VEncondig Write VEncondig;
    // Encoding da string
    Property Context: string Read vContentex Write vContentex; // Contexto
    Property RESTContext: string Read vRESTContext Write vRESTContext;
    // Rest Contexto
...
Type
  ...
  Private
    ...
    // Tabela que será feito Update no Servidor se for usada Reflexão de Dados
    vCacheUpdateRecords, vReadData, vCascadeDelete, vBeforeClone, vDataCache,
    // Se usa cache local
    vConnectedOnce, // Verifica se foi conectado ao Servidor
    vCommitUpdates, vCreateDS, vErrorBefore, vActive: Boolean;
    // Estado do Dataset
    ...
    vOnGetDataError: TOnEventConnection;
    // Se deu erro na hora de receber os dados ou não
    ...
    // Recebe os Dados da Internet vindo do Servidor REST
    Procedure SetUpdateTableName(Value: String);
    // Diz qual a tabela que será feito Update no Banco
    Procedure OldAfterPost(DataSet: TDataSet);
    // Eventos do Dataset para realizar o AfterPost
    Procedure OldAfterDelete(DataSet: TDataSet);
    // Eventos do Dataset para realizar o AfterDelete
  ...
  Public
    ...
    Procedure Open(SQL: String); Overload; Virtual;
    // Método Open que será utilizado no Componente
    ...
    procedure Refresh;
  Published
    Property MasterDataSet: TRESTClientSQL Read vMasterDataSet
      Write SetMasterDataSet;
    Property MasterCascadeDelete: Boolean Read vCascadeDelete
      Write vCascadeDelete;
    Property AfterDelete: TDataSetNotifyEvent Read vOnAfterDelete
      Write vOnAfterDelete;
    Property OnGetDataError: TOnEventConnection Read vOnGetDataError
      Write vOnGetDataError; // Recebe os Erros de ExecSQL ou de GetData
    Property AfterScroll: TOnAfterScroll Read vOnAfterScroll
      Write vOnAfterScroll;
    ...
    Property AfterInsert: TOnAfterInsert Read vOnAfterInsert
      Write vOnAfterInsert;
    Property BeforeDelete: TOnBeforeDelete Read vOnBeforeDelete
      Write vOnBeforeDelete;
    ...
    Property DataBase: TRESTDataBase Read vRESTDataBase Write SetDataBase;
    // Database REST do Dataset
    Property SQL: TStringList Read vSQL Write SetSQL; // SQL a ser Executado
    Property UpdateTableName: String Read vUpdateTableName
      Write SetUpdateTableName; // Tabela que será usada para Reflexão de Dados
    Property CacheUpdateRecords: Boolean Read vCacheUpdateRecords
      Write SetCacheUpdateRecords;
  End;
...
Type
  ...
  Published
    ...
    Property Params: TParams Read vParams Write vParams;
    // Parametros de Dataset
    Property ProcName: String Read vProcName Write vProcName;
...
Type
  ...
  Private
    ...
    vPoolerPort: Integer; // A Porta do Pooler
    vConnected, vProxy: Boolean; // Diz se tem servidor Proxy
    vProxyOptions: TProxyOptions; // Se tem Proxy diz quais as opções
  ...
  Published
    ...
    Property PoolerPort: Integer Read vPoolerPort Write SetPoolerPort;
    // A Porta do Pooler do DataSet
    Property PoolerPrefix: String Read vPoolerPrefix Write vPoolerPrefix;
    // Prefixo do WebService REST
...
Type
  ...
  Private
    vStrsTrim, vStrsEmpty2Null, vStrsTrim2Len, vCompression: Boolean;
  ...
  Public
    ...
    Procedure ApplyChanges(TableName, SQL: String; Var Error: Boolean;
      Var MessageError: String; Const ADeltaList: TFDJSONDeltas); Overload;
      Virtual; abstract;
    ...
    Function InsertMySQLReturnID(SQL: String; Params: TParams;
      ...
      Virtual; abstract;
    Procedure ExecuteProcedure(ProcName: String; Params: TParams;
      Var Error: Boolean; Var MessageError: String); Virtual; abstract;
...
Type
  ...
  Private
    ...
    vRESTDriverBack, vRESTDriver: TRESTDriver;
    vActive, vStrsTrim, vStrsEmpty2Null, vStrsTrim2Len, vCompression: Boolean;
    vEncoding: TEncodeSelect;
  ...
  Public
    ...
    Procedure ApplyChanges(TableName, SQL: String; Var Error: Boolean;
      Var MessageError: String; Const ADeltaList: TFDJSONDeltas); Overload;
    Function ExecuteCommand(SQL: String; Var Error: Boolean;
      Var MessageError: String; Execute: Boolean = False)
      : TFDJSONDataSets; Overload;
    Function ExecuteCommand(SQL: String; Params: TParams; Var Error: Boolean;
      Var MessageError: String; Execute: Boolean = False)
      : TFDJSONDataSets; Overload;
    Function InsertMySQLReturnID(SQL: String; Var Error: Boolean;
  ...
  Published
    ...
    Property PoolerOffMessage: String Read vMessagePoolerOff
      Write vMessagePoolerOff;
  End;
{$ENDIF}
...
Function TRESTPoolerDB.InsertMySQLReturnID(SQL: String; Var Error: Boolean;
...
Function TRESTPoolerDB.InsertMySQLReturnID(SQL: String; Params: TParams;
  Var Error: Boolean; Var MessageError: String): Integer;
Begin
  Result := -1;
  If vRESTDriver <> Nil Then
...
Function TRESTPoolerDB.ExecuteCommand(SQL: String; Var Error: Boolean;
  Var MessageError: String; Execute: Boolean = False): TFDJSONDataSets;
Begin
  Result := nil;
  If vRESTDriver <> Nil Then
...
Function TRESTPoolerDB.ExecuteCommand(SQL: String; Params: TParams;
  Var Error: Boolean; Var MessageError: String; Execute: Boolean = False)
  : TFDJSONDataSets;
Begin
  Result := Nil;
  ...
  Begin
    ...
    vRESTDriver.vCompression := vCompression;
    vRESTDriver.vEncoding := vEncoding;
    Result := vRESTDriver.ExecuteCommand(SQL, Params, Error,
...
Procedure TRESTPoolerDB.ExecuteProcedure(ProcName: String; Params: TParams;
  Var Error: Boolean; Var MessageError: String);
Begin
  If vRESTDriver <> Nil Then
  Begin
...
Procedure TRESTPoolerDB.ApplyChanges(TableName, SQL: String; Var Error: Boolean;
...
Procedure TRESTPoolerDB.ApplyChanges(TableName, SQL: String; Params: TParams;
  Var Error: Boolean; Var MessageError: String;
...
begin
  ...
  Begin
    ...
    vRESTDriver.vCompression := vCompression;
    vRESTDriver.vEncoding := vEncoding;
    vRESTDriver.ApplyChanges(TableName, SQL, Params, Error, MessageError,
      ADeltaList);
  End
  Else
  Begin
...
Procedure TRESTDataBase.ApplyUpdates(Var SQL: TStringList; Var Params: TParams;
  ADeltaList: TFDJSONDeltas; TableName: String; Var Error: Boolean;
  Var MessageError: String);
Var
...
Begin
  ...
  Try
    If Params.Count > 0 Then
      vRESTConnectionDB.ApplyChanges(vRestPooler, vRestModule, TableName,
        GetLineSQL(SQL), Params, ADeltaList, Error, MessageError, '', vTimeOut,
        vLogin, vPassword)
    Else
      vRESTConnectionDB.ApplyChangesPure(vRestPooler, vRestModule, TableName,
        GetLineSQL(SQL), ADeltaList, Error, MessageError, '', vTimeOut, vLogin,
        vPassword);
...
Function TRESTDataBase.InsertMySQLReturnID(Var SQL: TStringList;
  Var Params: TParams; Var Error: Boolean; Var MessageError: String): Integer;
Var
  vDSRConnection: TDSRestConnection;
  vRESTConnectionDB: TSMPoolerMethodClient;
  oJsonObject: Integer;
...
Begin
  ...
  Try
    If Params.Count > 0 Then
      oJsonObject := vRESTConnectionDB.InsertValue(vRestPooler, vRestModule,
        GetLineSQL(SQL), Params, Error, MessageError, '', vTimeOut, vLogin,
        vPassword)
    Else
      oJsonObject := vRESTConnectionDB.InsertValuePure(vRestPooler, vRestModule,
        GetLineSQL(SQL), Error, MessageError, '', vTimeOut, vLogin, vPassword);
    Result := oJsonObject;
    If Assigned(vOnEventConnection) Then
...
Function TRESTDataBase.ExecuteCommand(Var SQL: TStringList; Var Params: TParams;
  Var Error: Boolean; Var MessageError: String; Execute: Boolean = False)
  : TFDJSONDataSets;
Var
  vDSRConnection: TDSRestConnection;
  ...
  Try
    If Params.Count > 0 Then
      oJsonObject := vRESTConnectionDB.ExecuteCommandJSON(vRestPooler,
        vRestModule, GetLineSQL(SQL), Params, Error, MessageError, Execute, '',
        vTimeOut, vLogin, vPassword)
    Else
      oJsonObject := vRESTConnectionDB.ExecuteCommandPureJSON(vRestPooler,
        vRestModule, GetLineSQL(SQL), Error, MessageError, Execute, '',
        vTimeOut, vLogin, vPassword);
      ...
      Begin
        ...
        Begin
          ...
          Try
            ...
            MemTable.AppendData(TFDJSONDataSetsReader.GetListValue
              (LDataSetList, 0));
            MemTable.First;
...
Begin
  ...
  Begin
    ...
    Try
      If Params.Count > 0 Then
        vRESTConnectionDB.ExecuteProcedure(vRestPooler, vRestModule, ProcName,
          Params, Error, MessageError)
      Else
        vRESTConnectionDB.ExecuteProcedurePure(vRestPooler, vRestModule,
          ProcName, Error, MessageError);
    Except
      On E: Exception do
      Begin
        vDSRConnection.SessionID := '';
...
Begin
  ...
  Try
    vTempList := vRESTConnectionDB.PoolersDataSet(vRestModule, '', vTimeOut,
      vLogin, vPassword);
    Try
...
Begin
  ...
  Try
    ...
    vPoolerList.Assign(vRESTConnectionDB.PoolersDataSet(vPoolerPrefix,
      vTempResult, 3000, vLogin, vPassword));
    Result := True;
  Except
    On E: Exception do
    Begin
...
Begin
  ...
  Try
    ...
    If csDesigning in ComponentState Then
      If Trim(vTempResult) = '' Then
        Raise Exception.Create
          (PChar('Error : ' + #13 + 'Authentication Error...'));
    If Trim(vMyIP) = '' Then
      If Assigned(vOnEventConnection) Then
...
Begin
  If (Value) And (Trim(vRestPooler) = '') Then
    Exit;
...
Procedure TRESTClientSQL.DynamicFilter(Field, Value: String;
...
Var
  ...
  Begin
    ...
    Begin
      Inc(FCurrentPos);
      if CharInSet(vOldChar, [' ', '=', '-', '+', '<', '>', '(', ')', ':', '|'])
      then
      Begin
        While Not(FCurrentPos^ = #0) Do
        Begin
          if CharInSet(FCurrentPos^, ['0' .. '9', 'A' .. 'Z', 'a' .. 'z', '_'])
          then
...
Begin
  ...
  While Not(FCurrentPos^ = #0) do
  Begin
    If Not CharInSet(FCurrentPos^, [#0 .. ' ', ',', '''', '"', '0' .. '9',
      'A' .. 'Z', 'a' .. 'z', '_', '$', #127 .. #255]) Then
...
Begin
  ...
  Begin
    ...
    Begin
      ...
      Begin
        vMasterDetailList.Items[I].ParseFields
          (TRESTClientSQL(vMasterDetailList.Items[I].DataSet).MasterFields);
...
Var
  ...
  Begin
    If (FindField(FieldName) <> Nil) And
      (Value.FindField(FieldName) <> Nil) Then
  ...
  Begin
    ...
    Begin
      ...
      Begin
        vFields.Add(Uppercase(Trim(Copy(vTempFields, 1,
          Pos(';', vTempFields) - 1))));
...
Begin
  ...
  If Assigned(vRESTDataBase) And (Trim(UpdateTableName) <> '') Then
    vRESTDataBase.ApplyUpdates(vSQL, vParams, LDeltaList,
      Trim(vUpdateTableName), vError, vMessageError)
  Else
  Begin
    vError := True;
...
Begin
  EffectThread.CreateAnonymousThread(
    Procedure
    Begin
...
Procedure TRESTClientSQL.CloneDefinitions(Source: TFDMemTable;
aSelf: TRESTClientSQL);
Var
  I, A: Integer;
Begin
  aSelf.Close;
  ...
  Begin
    For A := 0 to aSelf.FieldDefs.Count - 1 do
      If Uppercase(Source.FieldDefs[I].Name)
        = Uppercase(aSelf.FieldDefs[A].Name) Then
      Begin
  ...
  Begin
    ...
    Try
      ...
      Begin
        ...
        Try
          Assert(TFDJSONDataSetsReader.GetListCount(LDataSetList) = 1);
          vTempTable.AppendData(TFDJSONDataSetsReader.GetListValue
            (LDataSetList, 0));
